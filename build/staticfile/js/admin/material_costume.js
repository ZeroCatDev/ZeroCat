...
var _table_render = table.render({
    elem: '#TableOne'
    , url: '/admin/material/costume/data'
    , where:{tagId:0}
    ...
    ,done: function(res, curr, count){
        ...
        upload.render({
            elem: '.costumeImage'
            ,url: '/admin/material/costume/modImage'
...
table.on('toolbar(TableOne)', function (obj) {
    if (obj.event === 'showMates') {
        ...
        layer.confirm(`添加一个造型？`,{title:'提示',shadeClose:true}, function (index) {
            AjaxFn('/admin/material/costume/add', {tagId: _tagId}, function (res){
                layer.msg(res.msg);
                if ('ok' == res.status){
                    _table_render.reload({where: {tagId: _tagId}, page:{curr:1}});
                    layer.close(index);
...
table.on('tool(TableOne)', function (obj) {
    if(obj.event === 'modImage'){
        ...
        layer.confirm(`删除 ${obj.data.name} ？`,{title:'重要提示',shadeClose:true}, function (index) {
            AjaxFn('/admin/material/costume/del', {id: obj.data.id}, function (res){
                layer.msg(res.msg);
                if ('ok' == res.status){
                    obj.del();
                    layer.close(index);
        ...
        AjaxFn('/admin/material/costume/setState', {id: obj.data.id, v: $(this).attr('data-v')}, function (res){
            layer.msg(res.msg);
            if ('ok' == res.status){
...
table.on('edit(TableOne)', function(obj){
    ...
    if (field == 'name') {
        AjaxFn('/admin/material/costume/modName', {id: obj.data.id, v:value}, function (res){
            layer.msg(res.msg);
            if ('ok' == res.status){
        ...
        AjaxFn('/admin/material/costume/modMateSize', {id: obj.data.id, t:field, v}, function (res){
            layer.msg(res.msg);
            if ('ok' == res.status){
                _table_render.reload({where: {tagId: _tagId}});
            }
