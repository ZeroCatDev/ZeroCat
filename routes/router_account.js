...
// 退出登录接口
router.post("/logout", strictTokenCheck, async function (req, res) {
  try {
...
// 退出所有设备
router.post("/logout-all-devices", strictTokenCheck, async function (req, res) {
  try {
    const tokenId = res.locals.tokenId;
...
// 获取令牌详情，包括地理位置信息
router.get("/token-details/:tokenId", needlogin, async function (req, res) {
  try {
    const userId = res.locals.userid;
...
// 获取活跃令牌列表
router.get("/active-tokens", needlogin, async function (req, res) {
  try {
    const userId = res.locals.userid;
...
// 吊销特定令牌
router.post("/revoke-token", needlogin, async function (req, res) {
  try {
