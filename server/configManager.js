...
class ConfigManager {
  ...
  getConfigSync(key) {
    ...
    throw new Error(`Config key "${key}" not found in cache. Use async getConfig() instead.`);
  }

  getPublicConfigSync(key) {
    // Check if the value is already cached
    if (global.publicconfig && global.publicconfig[key]) {
      return global.publicconfig[key]; 
    }
    throw new Error(`Public config key "${key}" not found in cache. Use async getPublicConfigs() instead.`);
  }
}
