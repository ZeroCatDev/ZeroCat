...
// 从配置中读取极验的相关信息
const { GEE_CAPTCHA_ID, GEE_CAPTCHA_KEY, GEE_API_SERVER } = global.config.captcha;
...
app.use(async (req, res, next) => {
    ...
    try {
        ...
        if (result.result === "success") {
            ...
            res.status(500).send({ code: 500, msg: `请完成验证码/${result.reason}`, status: `请完成验证码/${result.reason}` });
        }
    } catch (error) {
