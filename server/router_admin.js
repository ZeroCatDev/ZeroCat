...
router.get("/user/data", function (req, res, next) {
  try {
    ...
    DB.query(SQL, function (err, count) {
      ...
      if (state == 9) {
        //0：正常用户，2：封号用户，9：查找用户
        SQL = `SELECT id,email,display_name,state,regTime FROM ow_users WHERE email LIKE '%${
          req.query.t
        }%' LIMIT ${(page - 1) * limit}, ${limit}`;
      } else {
        SQL = `SELECT id,email,display_name,state,regTime FROM ow_users WHERE state=${state} LIMIT ${
          (page - 1) * limit
        }, ${limit}`;
      }
      DB.query(SQL, function (err, data) {
...
//管理员重置用户密码
router.post("/user_setpassword", function (req, res, next) {
  try {
...
//用户管理：功能.0解封 2封号
router.post("/user_setstate", function (req, res, next) {
  try {
    var state = 0;
...
//用户管理：创建新用户，功能
router.post("/user_new", function (req, res, next) {
  try {
...
//用户管理：批量创建新用户，功能
router.post("/user_new100", function (req, res, next) {
  try {
...
//用户管理：开启、关闭用户注册通道
router.post("/user/setRegist", function (req, res, next) {
  try {
    let v = 1;
    if (req.body.v != 1) {
      v = 0;
...
//作品管理：Scratch页面
router.get("/works/scratch", function (req, res, next) {
  try {
...
//作品管理：Scratch数据
router.get("/works/scratch/data", function (req, res, next) {
  try {
    WHERE = "";
    NICKNAME = ""; //根据昵称查找
    ...
    if (NICKNAME == "") {
      ...
      var SQL =
        `SELECT count(scratch.id) AS c FROM scratch ` +
    ...
    DB.query(SQL, function (err, count) {
      ...
      var limit = parseInt(req.query["limit"]);
      SQL =
        `SELECT scratch.id, scratch.state, scratch.title, scratch.time, ow_users.email, ow_users.display_name FROM scratch ` +
        ` INNER JOIN ow_users ON (ow_users.id=scratch.authorid ${NICKNAME}) ` +
        ` ${WHERE} ORDER BY scratch.time DESC LIMIT ${
          (page - 1) * limit
        },${limit}`;
...
//作品管理：设置作品的标题
router.post("/works/scratch/changeTitle", function (req, res, next) {
  try {
...
//作品管理：设置作品的发布状态
router.post("/works/scratch/setState", function (req, res, next) {
  try {
...
//作品管理：复制作品为默认作品
router.post("/works/scratch/setDefaultWork", function (req, res, next) {
  try {
...
//作品管理：Python页面
router.get("/works/python", function (req, res, next) {
  try {
...
//作品管理：Python数据
router.get("/works/python/data", function (req, res, next) {
  try {
    WHERE = "";
    NICKNAME = ""; //根据昵称查找
    if (req.query.w == "search_state0") {
    ...
    if (NICKNAME == "") {
      ...
      var SQL =
        `SELECT count(python.id) AS c FROM python ` +
    ...
    DB.query(SQL, function (err, count) {
      ...
      var limit = parseInt(req.query["limit"]);
      SQL =
        `SELECT python.id, python.state, python.title, python.time, ow_users.email, ow_users.display_name FROM python ` +
        ` INNER JOIN ow_users ON (ow_users.id=python.authorid ${NICKNAME}) ` +
        ` ${WHERE} ORDER BY python.time DESC LIMIT ${
          (page - 1) * limit
        },${limit}`;
...
//作品管理：Python 设置作品的标题
router.post("/works/python/changeTitle", function (req, res, next) {
  try {
...
router.post("/works/python/setState", function (req, res, next) {
  try {
    if (req.body.s == undefined || req.body.s < 0 || 2 < req.body.s) {
      s = 0; //未知时，都当作取消推荐处理
    } else {
...
router.post("/works/python/setDefaultWork", function (req, res, next) {
  try {
    if (!req.body.id || req.body.id == 1) {
      // 客服端传参数问题
      res.status(200).send({ status: "ok", msg: "操作成功" });
...
// 素材分类管理：数据
router.get("/material/tag", function (req, res, next) {
  try {
...
// 素材分类管理：数据
router.get("/material/tag/data", function (req, res, next) {
  try {
    var type = parseInt(req.query["t"]);
    if (!type || type < 1 || 4 < type) {
      // 1背景、2角色、3造型、4声音
...
// 素材分类管理：添加
router.post("/material/tag/add", function (req, res, next) {
  try {
    ...
    DB.query(SQL, function (err, newTag) {
      ...
      res
        .status(200)
        .send({ status: "ok", msg: "操作成功", newTagId: newTag.insertId });
    });
  } catch (err) {
    next(err);
  }
...
// 素材分类管理：修改
router.post("/material/tag/mod", function (req, res, next) {
  try {
    if (!req.body.v || req.body.v == "") {
      res.status(200).send({ status: "x", msg: "标签名错误" });
      return;
...
// 素材分类管理：删除
router.post("/material/tag/del", function (req, res, next) {
  try {
    // 判断此标签下是否有素材：只能删除空标签
    var type = parseInt(req.body.t);
    var tabelname = ""; // 1背景、2角色、3造型、4声音
    if (type == 1) {
      tabelname = "material_backdrop";
    } else if (type == 2) {
      tabelname = "material_sprite";
    } else if (type == 3) {
      tabelname = "material_costume";
    } else if (type == 4) {
      tabelname = "material_sound";
    } else {
      res.status(200).send({ status: "x", msg: "素材类型参数错误" });
      return;
    }
    ...
    DB.qww(SQL, VAL, function (err, MATE) {
      ...
      DB.qww(SQL, VAL, function (err, TAG) {
        if (err) {
          res
            .status(200)
            .send({ status: "x", msg: "保存数据错误，请再试一次" });
          return;
        }
...
// 背景管理：页面
router.get("/material/backdrop", function (req, res, next) {
  try {
...
// 背景管理：数据
router.get("/material/backdrop/data", function (req, res, next) {
  try {
    ...
    DB.qww(SQL, VAL, function (err, count) {
      ...
      var page = parseInt(req.query["page"]);
      var limit = parseInt(req.query["limit"]);
      SQL = `SELECT * FROM material_backdrop WHERE tagId=? ORDER BY id DESC LIMIT ${
        (page - 1) * limit
      },${limit}`;
...
// 背景管理：添加
router.post("/material/backdrop/add", function (req, res, next) {
  try {
    ...
    DB.query(SQL, function (err, newTag) {
      ...
      fs.copyFile(exampleFile, md5File, function (err) {
        if (err) {
        }
      });
...
// 背景管理：修改素材
router.post("/material/backdrop/modImage", function (req, res, next) {
  try {
    if (!req["files"]["file"]) {
    ...
    if (oldExt == newExt) {
      ...
      // 删除原图片文件
      fs.unlink(`./data/material/asset/${md5}`, function (err) {
        if (err) {
          console.log(err);
        }
      });
...
// 背景管理：修改名称
router.post("/material/backdrop/modName", function (req, res, next) {
  try {
    if (!req.body.v || req.body.v == "") {
...
// 背景管理：修改尺寸
router.post("/material/backdrop/modMateSize", function (req, res, next) {
  try {
    ...
    if (req.body.t == "info0" || req.body.t == "info1") {
      if (!infoValue || infoValue < 1 || 9999 < infoValue) {
        res
          .status(200)
          .send({ status: "x", msg: "参数错误：宽、高只能在 1~9999 之间" });
        return;
      }
...
router.post("/material/backdrop/del", function (req, res, next) {
  try {
    var VAL = [req.body.id];
...
// 背景管理：状态管理
router.post("/material/backdrop/setState", function (req, res, next) {
  try {
    let v = 0; //默认都当作 停用 处理
...
// 造型管理：页面
router.get("/material/costume", function (req, res, next) {
  try {
...
// 造型管理：数据
router.get("/material/costume/data", function (req, res, next) {
  try {
    var VAL = [req.query.tagId];
    ...
    DB.qww(SQL, VAL, function (err, count) {
      ...
      var page = parseInt(req.query["page"]);
      var limit = parseInt(req.query["limit"]);
      SQL = `SELECT * FROM material_costume WHERE tagId=? ORDER BY id DESC LIMIT ${
        (page - 1) * limit
      },${limit}`;
      DB.qww(SQL, VAL, function (err, data) {
        if (err) {
...
// 造型管理：添加
router.post("/material/costume/add", function (req, res, next) {
  try {
    var newFileName = random_32ID_With_Time_Tag();
...
// 造型管理：修改素材文件
router.post("/material/costume/modImage", function (req, res, next) {
  try {
    if (!req["files"]["file"]) {
      res.status(200).send({ status: "x", msg: "文件上传失败,请再试一次" });
      return;
...
// 造型管理：修改名称
router.post("/material/costume/modName", function (req, res, next) {
  try {
    if (!req.body.v || req.body.v == "") {
      res.status(200).send({ status: "x", msg: "名称错误" });
      return;
...
// 造型管理：修改素材属性：尺寸
router.post("/material/costume/modMateSize", function (req, res, next) {
  try {
    let infoValue = parseInt(req.body.v);
    if (req.body.t == "info0" || req.body.t == "info1") {
      if (!infoValue || infoValue < 1 || 9999 < infoValue) {
...
router.post("/material/costume/del", function (req, res, next) {
  try {
...
// 造型管理：素材状态管理
router.post("/material/costume/setState", function (req, res, next) {
  try {
    let v = 0; //默认都当作 停用 处理
    if (req.body.v && req.body.v == 1) {
      v = 1;
...
// 角色管理：页面
router.get("/material/sprite", function (req, res, next) {
  try {
...
// 角色管理：页面数据
router.get("/material/sprite/data", function (req, res, next) {
  try {
    var VAL = [req.query.tagId];
    ...
    DB.qww(SQL, VAL, function (err, count) {
      ...
      var page = parseInt(req.query["page"]);
      var limit = parseInt(req.query["limit"]);
      SQL = `SELECT * FROM material_sprite WHERE tagId=? ORDER BY id DESC LIMIT ${
        (page - 1) * limit
      },${limit}`;
      DB.qww(SQL, VAL, function (err, data) {
        if (err) {
...
// 角色管理：添加
router.post("/material/sprite/add", function (req, res, next) {
  try {
...
router.post("/material/sprite/del", function (req, res, next) {
  try {
    // 需要判断是否在角色中有被使用到，有被角色使用到时，不能删除
    var VAL = [req.body.id];
...
// 角色管理：导入角色：作品数据
router.get("/material/sprite/worklist", function (req, res, next) {
  try {
    var _title = "";
    ...
    DB.query(SQL, function (err, count) {
      ...
      var page = parseInt(req.query["page"]);
      var limit = parseInt(req.query["limit"]);
      var SQL = `SELECT id,title FROM scratch WHERE authorid=${
        res.locals.userid
      } ${_title} ORDER BY time DESC LIMIT ${(page - 1) * limit}, ${limit}`;
      DB.query(SQL, function (err, data) {
        if (err) {
          res.status(200).send({ count: 0, data: [] });
        } else {
...
router.post("/material/sprite/import", function (req, res, next) {
  try {
    // 1、获取作品src
    ...
    DB.query(SELECT, function (err, SRC) {
      ...
      for (var i = 0; i < targets.length; i++) {
        if (!targets[i].isStage) {
          ...
          values += `(${req.body.tagid}, '${targets[i].name}', '${JSON.stringify(
            json
          ).replace(/'/g, "’")}')`;
      ...
      DB.query(SQL, function (err, D) {
        ...
        res
          .status(200)
          .send({ status: "ok", msg: `操作成功，导入角色数：${spriteCount}` });
      });
    });
  } catch (err) {
    next(err);
...
// 角色管理：修改名称
router.post("/material/sprite/modName", function (req, res, next) {
  try {
    if (!req.body.v || req.body.v == "") {
      res.status(200).send({ status: "x", msg: "名称错误" });
      return;
...
// 角色管理：素材状态管理
router.post("/material/sprite/setState", function (req, res, next) {
  try {
    let v = 0; //默认都当作 停用 处理
    if (req.body.v && req.body.v == 1) {
      v = 1;
...
// 角色管理：造型管理 页面
router.get("/material/sprite/setCostume", function (req, res, next) {
  try {
...
// 角色管理：造型管理 页面数据
router.get("/material/sprite/setCostume/data", function (req, res, next) {
  try {
...
router.post("/material/sprite/setCostume/del", function (req, res, next) {
  try {
    // 需要判断是否在角色中有被使用到，有被角色使用到时，不能删除
    ...
    DB.qww(SQL, VAL, function (err, D) {
      ...
      SQL = `UPDATE material_sprite SET json='${JSON.stringify(json)}' WHERE id=${
        req.body.id
      }`;
...
// {text: 搜索文本，默认为空：''}
router.get("/material/sprite/setCostume/select", function (req, res, next) {
  try {
    ...
    DB.query(SQL, function (err, count) {
      ...
      var limit = parseInt(req.query["limit"]);
      SQL =
        `SELECT c.id, c.name, t.tag FROM material_costume c ` +
        ` LEFT JOIN material_tags t ON (c.tagId = t.id) ` +
        ` WHERE c.name LIKE '%${req.query.text}%' LIMIT ${
          (page - 1) * limit
        },${limit}`;
      DB.query(SQL, function (err, data) {
        if (err) {
...
router.post("/material/sprite/setCostume/add", function (req, res, next) {
  try {
    // 取造型数据
    let SQL =
      `SELECT c.name, c.md5, c.info0, c.info1, c.info2, s.json FROM material_costume c` +
...
// 角色管理：声音管理 页面
router.get("/material/sprite/setSound", function (req, res, next) {
  try {
    res.locals._id = req.query["id"];
...
// 角色管理：声音管理 页面 数据
router.get("/material/sprite/setSound/data", function (req, res, next) {
  try {
    var SQL = `SELECT json FROM material_sprite WHERE id=${req.query["id"]} LIMIT 1`;
    DB.query(SQL, function (err, D) {
      if (err || D.length == 0) {
...
// 参数：{id: _id, index: obj.data.index, md5: obj.data.md5ext}
router.post("/material/sprite/setSound/del", function (req, res, next) {
  try {
    // 需要判断是否在角色中有被使用到，有被角色使用到时，不能删除
    var SQL = `SELECT json FROM material_sprite WHERE id=?`;
    var VAL = [req.body.id];
...
// {text: 搜索文本，默认为空：''}
router.get("/material/sprite/setSound/select", function (req, res, next) {
  try {
    ...
    DB.query(SQL, function (err, count) {
      ...
      var limit = parseInt(req.query["limit"]);
      SQL =
        `SELECT c.id, c.name, t.tag FROM material_sound c ` +
        ` LEFT JOIN material_tags t ON (c.tagId = t.id) ` +
        ` WHERE c.name LIKE '%${req.query.text}%' LIMIT ${
          (page - 1) * limit
        },${limit}`;
...
router.post("/material/sprite/setSound/add", function (req, res, next) {
  try {
    // 取声音数据
    let SQL =
      `SELECT c.name, c.md5, c.format, c.rate, c.sampleCount, s.json FROM material_sound c` +
      ` LEFT JOIN material_sprite s ON (s.id=?) ` +
      ` WHERE c.id=?`;
    let VAL = [req.body.sid, req.body.cid];
    DB.qww(SQL, VAL, function (err, D) {
...
// 声音管理：页面
router.get("/material/sound", function (req, res, next) {
  try {
...
// 声音管理：数据
router.get("/material/sound/data", function (req, res, next) {
  try {
    var VAL = [req.query.tagId];
    ...
    DB.qww(SQL, VAL, function (err, count) {
      ...
      var page = parseInt(req.query["page"]);
      var limit = parseInt(req.query["limit"]);
      SQL = `SELECT * FROM material_sound WHERE tagId=? ORDER BY id DESC LIMIT ${
        (page - 1) * limit
      },${limit}`;
      DB.qww(SQL, VAL, function (err, data) {
        if (err) {
...
// 声音管理：添加
router.post("/material/sound/add", function (req, res, next) {
  try {
    // 仅仅支持wav格式的声音素材
...
// 声音管理：修改素材文件
router.post("/material/sound/modWav", function (req, res, next) {
  try {
    if (!req["files"]["file"]) {
      res.status(200).send({ status: "x", msg: "文件上传失败,请再试一次" });
      return;
...
// 声音管理：修改名称
router.post("/material/sound/modName", function (req, res, next) {
  try {
    if (!req.body.v || req.body.v == "") {
      res.status(200).send({ status: "x", msg: "名称错误" });
      return;
...
router.post("/material/sound/modMateAttr", function (req, res, next) {
  try {
    ...
    if (!infoValue || infoValue < 0 || 10000000 < infoValue) {
      res
        .status(200)
        .send({ status: "x", msg: "参数错误：只能在 0~1000,0000 之间" });
      return;
    }
...
router.post("/material/sound/del", function (req, res, next) {
  try {
    // 需要判断是否在角色中有被使用到，有被角色使用到时，不能删除
    var VAL = [req.body.id];
...
// 声音管理：素材状态管理
router.post("/material/sound/setState", function (req, res, next) {
  try {
    let v = 0; //默认都当作 停用 处理
    if (req.body.v && req.body.v == 1) {
      v = 1;
...
// 清理scratch缩略图
router.get("/clear_scratch_slt", function (req, res, next) {
  try {
    ...
    DB.query(SQL, function (err, data) {
      ...
      for (var i = data.length - 1; i >= 0; i--) {
        ...
        fs.copyFile(slt_old, slt, function (err) {
          if (err) {
          }
        });
      }
