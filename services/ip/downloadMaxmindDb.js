...
async function loadMaxmindConfigFromDB() {
  try {
    ...
    // 更新配置对象
    if (enabled !== null) USER_CONFIG.enabled = enabled === 'true' || enabled === '1';
...
async function downloadDatabase() {
  ...
  try {
    ...
    const tempFile = path.join(DATA_DIR, `${DOWNLOAD_CONFIG.edition}.${DOWNLOAD_CONFIG.suffix}`);
    ...
    data.on('data', (chunk) => {
      ...
      if (totalSize && percent % 10 === 0 && percent > lastLoggedPercent) {
        ...
        if (process.stdout.isTTY) {
          ...
          process.stdout.write(`下载进度: ${percent}% [${downloadedMB}MB / ${totalMB}MB]`);
        }
    ...
    if (process.stdout.isTTY) {
      process.stdout.clearLine(0);
      process.stdout.cursorTo(0);
      process.stdout.write(`下载完成: 100% [${(downloadedSize / 1024 / 1024).toFixed(2)}MB]\n`);
    }
    ...
    logger.error('下载或解压MaxMind数据库失败:', error.message);
    if (error.response) {
  ...
  try {
    ...
    try {
      ...
      await tar.x({
        ...
        cwd: extractDir,
        gzip: true,
        onentry: showProgress ? (entry) => {
          // 更新最后解压的文件名
          process.stdout.clearLine(0);
          process.stdout.cursorTo(0);
          const fileName = entry.path.split('/').pop();
          const elapsedSec = ((Date.now() - extractStartTime) / 1000).toFixed(1);
          process.stdout.write(`解压: ${fileName.slice(0, 40).padEnd(40)} [${elapsedSec}秒]`);
        } : undefined
      });
    } catch (error) {
      // 如果上面的方法失败，尝试命令行方式
...
async function main() {
  try {
    ...
    // 检查必要配置
    if (!USER_CONFIG.licenseKey) {
      console.error('错误: 未找到MaxMind许可证密钥。请在数据库中设置 maxmind.license_key 配置项。');
      process.exit(1);
    }

    if (!USER_CONFIG.accountId) {
      console.error('错误: 未找到MaxMind账户ID。请在数据库中设置 maxmind.account_id 配置项。');
      process.exit(1);
    }

    // 开始下载
    console.log(`开始下载MaxMind GeoIP数据库...`);
    const success = await downloadDatabase();

    if (success) {
      console.log('✓ MaxMind GeoIP数据库下载并更新成功！');
      process.exit(0);
    } else {
      console.error('✗ MaxMind GeoIP数据库下载或更新失败！');
      process.exit(1);
    }
  } catch (error) {
