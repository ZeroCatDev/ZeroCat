...
function createStateStore() {
    const prefix = 'atproto_oauth_state:auth:';
    return {
        async set(key, value) {
...
export async function buildAtprotoAuthClientMetadata(scope) {
...
async function createAuthClient(scope) {
    const clientMetadata = await buildAtprotoAuthClientMetadata(scope);
...
export async function createAtprotoAuthAuthorizeUrl({ loginHint, state, scope }) {
    ...
    const client = await createAuthClient(scope);
    try {
...
export async function consumeAtprotoAuthCallback({ callbackQuery, scope }) {
    const client = await createAuthClient(scope);
